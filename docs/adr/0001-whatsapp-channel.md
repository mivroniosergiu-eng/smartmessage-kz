...
- Общая lifecycle job остаётся durable coordinator до подтверждения owner-specific job. При ack-timeout она заново читает ownership; directed job несёт ожидаемые owner+epoch и становится no-op при смене generation. Поэтому падение owner не теряет команду, а осиротевший старый stop/logout/renew не применяется к новой сессии. Logout без активного owner кратко занимает fenced ownership и очищает сохранённый auth-state без открытия socket.
...
- `restricted` сохраняет auth-state, закрывает transport без logout, фиксирует монотонный абсолютный `restrictedUntil` и освобождает ownership только после durable delayed recovery enqueue. PostgreSQL остаётся источником истины: recovery job сверяет точный timestamp, а startup reconciliation восстанавливает jobs после crash-gap. `banned` — монотонный терминальный DB-статус, блокирующий start и новый ownership до открытия socket; переход создаёт один санитизированный `AuditLog`.
