...
- **Receiver**: входящие `messages.upsert`/`update` → стабильные типизированные batch-события `wa.message.upsert` / `wa.message.update`. Domain snapshot содержит только нормализованные ключи, направление, timestamp, тип/текст контента и status update; raw Baileys payload наружу не протекает. События старой socket-generation отбрасываются, ошибка consumer изолируется от последующих событий. Receiver сам не отправляет ответы и не персистит данные.
...
- **Sticky single-process ownership WA-сокетов.** Жизненный цикл Baileys-сокета одного `instanceId` принадлежит ровно одному физическому процессу на всём протяжении (long-lived stateful ресурс — короткие блокировки с TTL здесь бессильны и ведут к сокет-шторму/«conflict: replaced»).
  ...
  - **Identity release только после безопасного close.** Graceful shutdown удаляет process lease лишь после успешного `shutdownAll()` и `close(true)` обоих lifecycle workers. При ошибке lease не удаляется, renewal прекращается, fatal termination вызывается до потенциально зависимого queue/Redis/Prisma cleanup, а replacement ждёт TTL.
